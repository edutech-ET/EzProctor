...
import ezProctorLogo from "../../assets/brand/ezproctor-logo.png";
...
function App() {
  ...
  if (activationState !== "activated") {
    return <main className="educator-login-shell"><section className="educator-login-card activation-card"><div className="login-brand"><img className="brand-logo" src={ezProctorLogo} alt="EzProctor Exam" /><div><strong>EzProctor™</strong><small>Exam</small></div></div>{activationState === "checking" ? <div className="login-checking"><span></span><p>Checking platform activation...</p></div> : <form onSubmit={activate}><p className="eyebrow">Platform licence</p><h1>Activate EzProctor™.</h1><p>Enter the activation key issued to your institution or organisation. Activation is required once per server installation.</p><label>Institution or organisation<input autoFocus required value={activationForm.institution} onChange={(event) => setActivationForm({ ...activationForm, institution: event.target.value })} placeholder="Example University or Organisation" /></label><label>Contact email<input type="email" value={activationForm.contactEmail} onChange={(event) => setActivationForm({ ...activationForm, contactEmail: event.target.value })} placeholder="it@example.org" /></label><label>Activation key<input required autoComplete="off" value={activationForm.key} onChange={(event) => setActivationForm({ ...activationForm, key: event.target.value.toUpperCase() })} placeholder="EZEDU-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX-XXXXX" /></label>{activationError && <div className="login-error">{activationError}</div>}<button className="button primary wide" type="submit">Activate platform</button><a className="activation-request" href="https://ezproctor.ejoetso.com/#start">Request a free education key or subscription quote</a></form>}<footer>Copyright © 2026 Ejoe Tso · All rights reserved</footer></section></main>;
  }

  if (authState !== "authenticated") {
    return <main className="educator-login-shell"><section className="educator-login-card"><div className="login-brand"><img className="brand-logo" src={ezProctorLogo} alt="EzProctor Exam" /><div><strong>EzProctor™</strong><small>Exam</small></div></div>{authState === "checking" ? <div className="login-checking"><span></span><p>Checking educator session...</p></div> : <form onSubmit={loginEducator}><p className="eyebrow">Educator access</p><h1>Welcome back.</h1><p>Sign in to manage exams, sessions, submissions, and grades.</p><label>Username<input autoFocus required autoComplete="username" value={loginForm.username} onChange={(event) => setLoginForm({ ...loginForm, username: event.target.value })} /></label><label>Password<input required type="password" autoComplete="current-password" value={loginForm.password} onChange={(event) => setLoginForm({ ...loginForm, password: event.target.value })} /></label>{loginError && <div className="login-error">{loginError}</div>}<button className="button primary wide" type="submit">Sign in to Educator Console</button></form>}<footer>Copyright © 2026 Ejoe Tso · All rights reserved</footer></section></main>;
  }

  return (
    <div className="educator-app">
      <aside className="app-nav">
        <div className="brand-lockup"><img className="brand-logo" src={ezProctorLogo} alt="EzProctor Exam" /><div><strong>EzProctor™</strong><small>Exam</small></div></div>
        ...
        <div className="nav-footer"><Status tone={socketState === "Live" ? "success" : "danger"}>{socketState}</Status><small>Educator Console</small><small>© 2026 Ejoe Tso · All rights reserved</small><button className="nav-logout" onClick={logoutEducator}>Sign out</button></div>
      </aside>
